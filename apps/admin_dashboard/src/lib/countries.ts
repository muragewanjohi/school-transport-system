...
/** Country names with ITU dialing codes for marketing / demo lead forms. */
export const COUNTRIES: readonly CountryOption[] = [
  ...
  { name: "Congo, Republic of the", dialCode: "+242" },
...
export function filterCountries(query: string): CountryOption[] {
  ...
  return COUNTRIES.filter(
    (c) => c.name.toLowerCase().includes(q) || c.dialCode.includes(q.replace(/\s/g, ""))
  );
}

/** Unique dial codes for the phone extension select, Kenya first. */
export function getDialCodeOptions(): string[] {
  const seen = new Set<string>();
  const ordered: string[] = [];
  const prefer = ["+254", "+256", "+255", "+250", "+251", "+257", "+211", "+27", "+234", "+233", "+1", "+44"];
...
export function formatInternationalPhone(dialCode: string, local: string): string {
  return `${dialCode}${normalizeLocalPhone(local)}`;
}
