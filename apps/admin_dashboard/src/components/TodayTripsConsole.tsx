...
import { mapTodayTripDisplayStatus } from "@/lib/tripDisplayStatus";

export type TodayTripsConsoleMode = "history" | "override";

interface TodayTripsConsoleProps {
  mode: TodayTripsConsoleMode;
}
...
export default function TodayTripsConsole({ mode }: TodayTripsConsoleProps) {
  const allowOverride = mode === "override";
  const pageTitle = mode === "override" ? "Override trip" : "Today's trip history";
  ...
  const joinedTrips = activeSchedulesToday.map(sched => {
    ...
    const computedStatus = mapTodayTripDisplayStatus(sched.departure_time, dbTrip);
    ...
    <div className="app-container">
      ...
      <main className="main-content">
        <header className="main-header">
          <div>
            <span className="header-subtitle">Trips</span>
      ...
      {/* Override trip modal */}
      {allowOverride && showModal && selectedRun && (
        <div style={{
          position: "fixed",
          inset: 0,
