...
export default function EditStudentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [beaconTag, setBeaconTag] = useState<{
    uuid: string;
    major: number;
    minor: number;
    mac: string | null;
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (studentJson.success && studentJson.data) {
        ...
        setBeaconTag(student.beacon_tag ?? null);
            ...
            <form onSubmit={handleFormSubmit} className="form-card">
              ...
              <div>
                ...
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    <label className="form-label">NFC Card Hash (legacy, optional)</label>
                    <input
                      type="text"
                      name="nfc_card_hash"
                  ...
                  <div className="form-group" style={{ gridColumn: "1 / -1" }}>
                    <label className="form-label">BLE Tag (read-only)</label>
                    {beaconTag ? (
                      <div
                        className="form-hint"
                        style={{
                      ...
                      >
                        <div>
                          <strong>UUID</strong> {beaconTag.uuid}
                        </div>
                        <div>
                          <strong>Major / Minor</strong> {beaconTag.major} / {beaconTag.minor}
                        </div>
                        <div>
                          <strong>MAC</strong> {beaconTag.mac || "—"}
                        </div>
                        <div>
                          <strong>Device</strong> {beaconTag.device_name || "—"}
                        </div>
                        <div>
                          <strong>Status</strong> {beaconTag.status}
                          {beaconTag.provisioned_at
                            ? ` · provisioned ${new Date(beaconTag.provisioned_at).toLocaleString()}`
                            : ""}
                        </div>
                      </div>
                    ) : (
                      <p className="form-hint">
                        No active BLE tag — provision via Driver App.
                      </p>
                    )}
                  </div>
