...
- **Driver stop navigation (Trip):** Navigate enters in-app nav mode on the embedded Google Map (follow live bus, emphasize polyline to next stop, show distance/ETA). Does not hand off to external Google Maps from the Trip action card. Full voice/lane TBT remains out of scope. Stops-tab per-stop Navigate may still use deep links.
- **Driver stop completion:** Pickup Students / DropOff Students opens a ~70–80% bottom drawer scoped to the arrived stop. Checked students → `Present`; **Complete Stop** sets still-Pending students at that stop to `Absent`, marks the stop completed, and advances the next stop on the map. Geofence-gated. Persisted absent notes are out of scope (status-only API).
