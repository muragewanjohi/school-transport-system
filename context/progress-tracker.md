...
| **Async / platform** | Fake `MethodChannel`s for BLE/location; never require real GPS/BLE hardware to mark a unit complete. |
...
- **BLE SDK provision + password lock (2026-09-15):** Spec extended for Driver Provision Tag + tenant 6-char device password. Migration `student_beacon_tags` + `tenant_configs` beacon columns. APIs `GET/POST /api/driver/beacon/{template,provision,revoke,students}`. Android `DxBeaconPlugin` MethodChannel. Flutter Provision Tag screen, `BeaconProvisionService`, trip `BleTripScanService` + confidence engine. BDD in [bdd.md](bdd.md).
- **CP35 GATT provision client (2026-09-18):** Vendor has no AAR. First-party `Cp35FrameCodec` / `Cp35Commands` / `Cp35GattClient` / `AndroidCp35Transport` wired into `DxBeaconPlugin`. Spec §11 updated. JVM unit tests + Dart MethodChannel tests. Physical 3-tag lock checklist remains post-merge pilot (not CI).
- **Provision PIN + student BLE UI (2026-09-18):** Mandatory 6-digit Provision PIN auto-set on real/demo/play-review create; emailed once; admin rotate on `/config`; driver template requires PIN. Student edit shows read-only active BLE tag fields.

- **Request Demo field errors (2026-09-18):** `/request-demo` validation highlights the failing field with a human message. The hidden honeypot no longer returns Zod “Too big: expected string to have <=0 characters”. BDD in [bdd.md](bdd.md) Status `passing` — `src/lib/demoRequestValidation.test.ts`, `src/app/api/demo-requests/route.test.ts`.

- **Trips menu split (2026-09-18):** Sidebar **Trips** has **Today's trip history** (no overrides) and **Override trip** (same board as former Today's Trips). `/routes/today-trips` redirects to override. BDD in [bdd.md](bdd.md) Status `passing` — `src/lib/schoolCampusNav.test.ts`, `src/lib/tripDisplayStatus.test.ts`.


- **Mapbox token removed from school config (2026-09-18):** Alerts & Geofencing no longer shows Mapbox Access Token. Live ETAs stay on the platform Google Maps key. Client config sanitizer omits `mapbox_access_token`. BDD in [bdd.md](bdd.md) Status `passing` — `src/app/api/config/beaconPin.test.ts`.


- **Route stop type removed (2026-09-18):** Stop details, the stages list, and the route planner no longer show Stop type. Pickup vs drop-off remains student assignment + trip direction; new stops still persist `stop_type = BOTH`. BDD in [bdd.md](bdd.md) Status `passing` — `src/app/api/stops/route.test.ts`, `src/app/api/stops/[id]/route.test.ts`, `src/lib/studentStopAssignment.test.ts`.

- **Student trip searchable dropdowns (2026-09-18):** Pick up trip and Drop off trip on student add/edit use the same searchable select as pickup/drop-off location (including None). BDD in [bdd.md](bdd.md) Status `passing` — `src/lib/studentTripSelect.test.ts`.

- **Optional bus maker/model and capacity (2026-09-18):** Fleet add/edit can save without Maker & Model or Seating Capacity. Compliance notify remains fleet-card only (no admin email). BDD in [bdd.md](bdd.md) Status `passing` — `src/lib/vehicleFields.test.ts`, `src/lib/vehicleCompliance.test.ts`.

- **BLE boarding technology spec (2026-09-15):** Committed BLE iBeacon as primary hands-free boarding; added [boarding-technology.md](boarding-technology.md) (CP35 facts, pickup/drop-off confirmation rules, field-test gate T1–T7, Phase 2+ gate/classroom). Synced [project-overview.md](project-overview.md), [architecture.md](architecture.md), [architecture-security.md](architecture-security.md), [ui-context.md](ui-context.md), [code-standards.md](code-standards.md), [ai-workflow-rules.md](ai-workflow-rules.md), [pre-school-readiness.md](pre-school-readiness.md), [parent-app.md](parent-app.md). Docs only — no app/API code. Next: CP35 field validation before implementation.
...
- **Scheduled trip roster resync (2026-08-28):** While a trip is still `scheduled`, driver `trip_manifests` re-aligns to live `students.schedule_ids` (driver trip list, trip-manifest GET, student trip assignment). Frozen after start. BDD in [bdd.md](bdd.md) Status `passing` — `src/lib/scheduledTripManifest.test.ts`.

- **Student location search Google-only (2026-08-28):** Home/stop location picker no longer calls Nominatim; search uses `GET /api/maps/places`. BDD in [bdd.md](bdd.md) Status `passing` — `src/lib/homeLocationSearch.test.ts`.
...
- Parent bottom nav Notifications (2026-08-20): Attendance tab replaced by **Notifications** (Home · Map · Notifications · Profile); Present/Absent remains on Home; nav badge uses unread count. BDD Status `passing` — `test/notifications_screen_test.dart`.
- Parent student information row (2026-08-20): Profile child card uses one **Student Information** chevron row → editable name/address/photo screen; school grade/admission read-only. RLS: parents may UPDATE own children (assignment columns blocked). BDD Status `passing` — `test/student_info_screen_test.dart`.
- Parent photo upload (2026-08-28): Camera/gallery uploads were failing because `avatars` storage had no SELECT policy (INSERT … RETURNING * aborted) and HMAC-only sessions could not write Storage. Added owner SELECT, `POST /api/parent/avatar` (HMAC, service role), and Flutter API-first upload with Auth session restore. BDD Status `passing` — `src/lib/parentAvatar.test.ts`, `src/app/api/parent/avatar/route.test.ts`, `apps/parent_app/test/parent_avatar_logic_test.dart`.
- Parent Home riding toggle (2026-08-20): Removed Today's Schedule from Home; Present/Absent is a switch gated to pickup trips before boarding. BDD Status `passing` — `test/parent_attendance_logic_test.dart`.
- Parent Home live card (2026-08-20): Home transit card uses live trip status/plate/driver/next stop/ETA (no Bus 12 / John Kamau / Kiambu placeholders); grade label no longer doubles "Grade". BDD Status `passing` — `test/parent_grade_label_test.dart`.
...
- Debug GPS replay (2026-08-18): Debug driver builds only (`kDebugMode`, not Play release) can **Replay demo GPS** from the trip overflow menu. Walks the active trip’s stops (Play Review corridor fallback), drives the driver UI and `POST /api/driver/telemetry` so campus-exit and 500 m parent notifications fire without a physical device. Tests: `apps/driver_app/test/gps_replay_test.dart`, `trip_control_drawer_test.dart`.
...
- **BLE pilot lock (manual, post GATT merge):** Re-provision 3 CP35 tags via Driver **Provision Tag**; verify DX-SMART rejects `DX1234` after lock ([boarding-technology.md](boarding-technology.md) §11 checklist). Apply migration `20260915120000_student_beacon_tags.sql` if not yet applied.
- **BLE trip auto-board wiring:** Hook `BleTripScanService` + `ble_boarding_confidence.dart` into active-trip attendance writes (confidence engine + parse tests already landing).
...
- Optional: Calendly/Cal.com embed on `/request-demo`.
...
- **Trip start / override / auto-Missed hour caps:** Today a never-started run is auto-**Delayed** after **10 minutes**. **Missed** is a console chip after **30 minutes** (not persisted). Override and Start Trip have **no hour cap** on the same calendar day (a 06:45 run can be started at 09:00 and overridden at 11:00; completed trips cannot be overridden). Confirm whether we should persist Missed after N hours or block late starts/overrides.

## Architecture Decisions

- **BLE iBeacon primary boarding (2026-09-15):** Hands-free BLE student tags + fixed-mount Android driver phone are the committed primary boarding/drop-off path. Manual checklist remains the safety fallback. NFC is legacy/non-primary. No bus wiring for v1. Parent notify only after confirmed attendance. Campus `SCHOOL_TO_HOME` roll-call stays manual SwitchListTile until a later unlock. Gate/classroom readers are Phase 2+. Spec: [boarding-technology.md](boarding-technology.md). Field validation (T2–T4) required before implementation.
- **BLE provision SDK + tag password (2026-09-15):** Production tag setup uses Driver App **Provision Tag** (Android MethodChannel). Vendor has no AAR — first-party CP35 GATT client. Tenant-scoped 6-char device password replaces factory `DX1234` so DX-SMART cannot reconfigure without OnTheBus. Trip scan uses standard iBeacon parse — no SDK on the hot path.
- **CP35 GATT provision (2026-09-18):** `Cp35FrameCodec` / `Cp35Commands` / `Cp35GattClient` replace MethodChannel stub. Spec §11 documents FFE0–FFE3, 30s auth window, NO-framed commands. JVM protocol + fake-transport tests; Dart MethodChannel fakes. Physical 3-tag lock checklist remains post-merge pilot (not CI).
...
- **Parent FCM apps:** Parent Android is `com.schooltrack.parent_app` in Firebase project `school-transport-system-f606a`. Parent iOS is `com.schooltrack.parentApp` (underscores are invalid in Firebase/Apple bundle IDs). iOS Runner uses Push entitlements (`aps-environment` development in Debug, production in Release/Profile) and waits for APNs before `getToken()`. `send-push` is deployed (JWT verification off, same as `send-sms` webhooks) with `apns-push-type: alert`. Do not reuse driver app IDs.
- **Google Maps across clients:** Admin uses Maps JavaScript API + Directions/Places. Student/stop/route location search uses `GET /api/maps/places` (Google only) — not OpenStreetMap Nominatim. Driver and Parent use `google_maps_flutter` with native `MAPS_API_KEY`. Road polylines for mobile come from `POST /api/maps/directions` (server key → Google Directions). Parent relocate search uses `GET /api/maps/places`. School `/config` Alerts & Geofencing does not collect a Mapbox token; `calculate-eta` uses Google Distance Matrix (tenant or platform `GOOGLE_MAPS_API_KEY`) with Haversine fallback.
...
- **Stop-gated boarding:** Drivers may board a student only inside that student’s `pickup_stop` geofence and drop off only inside `dropoff_stop` geofence (radius = `stops.geofence_radius_meters`, default 50 m). Enforced in Driver UI and on driver attendance `PUT /api/students/[id]`. Exception: drop-off campus roll-call before Start Trip writes `trip_manifests` directly. When BLE auto-confirm ships, the same geofence + movement rules in [boarding-technology.md](boarding-technology.md) still apply — BLE does not bypass stop gates.
- **Route stops are corridor locations (2026-09-18):** A stop is a named geofenced point on a route. Pickup vs drop-off is the student’s `pickup_stop_id` / `dropoff_stop_id` plus trip direction (`HOME_TO_SCHOOL` / `SCHOOL_TO_HOME`). Admin stop details do not collect a Stop type. The `stops.stop_type` column remains `NOT NULL DEFAULT BOTH` for schema compatibility.
...
- **Today's trip status override:** Admin Update Status patches the daily `trips` row (`trip_id`), not the corridor. Completed trips are locked (409). Parent alerts match `students.schedule_ids` to that trip's schedule. School console **Trips** menu: `/trips/history` (read-only) and `/trips/override`. No hour-of-day cap on start or override for the same `trip_date`.
...
- **Driver Trip control drawer:** Active Trip tab is a bus-centered map (zoom ~15.8) plus a persistent bottom drawer (picked count, stop segments, next stop, Pickup/DropOff, overflow Navigate, Skip stop, Hold to end trip). Removed the trip-in-progress card, below-map next-stop card, END TRIP button, and Pickup Students FAB. BDD: *Drawer shows attendance, next stop, and pickup CTA*; *Hold to end trip requires a press-and-hold* (`apps/driver_app/test/trip_control_drawer_test.dart`).
- **Stop approach, dwell, leave, and two parent alerts:** Approaching vs You’re at drawer copy. `min_stop_dwell_seconds` (default 90, 60–180) gates Skip and auto-visited. Complete Stop or leave-with-ticks → completed + remaining Pending → Absent. Leave with zero ticks only after min dwell → visited + Absent + admin alert. Parents get two approach **notifications** (campus-exit ETA, then 500 m); SMS only if the school enabled it (`parentTripAlerts.test.ts`, `stop_visit_logic_test.dart`). Demo SMS dry-run unchanged.
