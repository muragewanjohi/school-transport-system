...
6. **Demo stores (`is_demo`):** Synthetic PII only. Operational trip/proximity SMS remains dry-run, but login OTP SMS is delivered to the registered demo phone through the configured SMS gateway so reviewers can complete authentication. Per-lead stores are named from the demo request, geo-shifted to the request city/area, and seeded with a slim roster (1 admin Auth user, 1 driver, 1 conductor, 3 guardians, 5 students + avatars). The request phone logs into Flutter parent + driver apps using a fresh six-digit OTP that expires after 15 minutes. If login SMS is rejected, OTP email fallback uses a real profile email, or for `is_demo` tenants the lead `tenants.contact_email` — never `@demo.onthebus.app` synthetic addresses. The permanent `play-review` tenant is the sole exception and keeps OTP `123456` for store reviewers. Default `demo_expires_at` is 14 days and is stored on both `tenants` and `demo_requests` so the Demos table and request detail always show the date. Platform can extend it from the demo request detail page (`/schools/demos/[id]`) and the Demos list while the store is `confirmed` or `ready_to_onboard` (future datetime only; `+14 days` adds from the later of now or the current expiry). If the tenant was already purged, those screens show that the store was removed and allow **Provision store again**. Complete or expiry hard-deletes the tenant (cascade) and Auth admin user.
...
- After login the app requests notification permission and `POST /api/parent/fcm-tokens` upserts `user_fcm_tokens`. **Android** rotates the FCM token on each login (stale tokens after reinstall return `UNREGISTERED`). **iOS** does not rotate: it waits for an APNs device token, then calls `getToken()`, and signs the Runner target with a Push Notifications entitlement (`aps-environment` development in Debug, production in Release/Profile). Logout `DELETE`s that token. `send-push` (on `notifications` INSERT via `trigger_push_webhook` → `net.http_post`) delivers lock-screen push when `FIREBASE_SERVICE_ACCOUNT` is set and tokens exist. `UNREGISTERED` / `NOT_FOUND` tokens are deleted so the next login can register a live one. Missing tokens skip push; the in-app row still exists. Push must work with the app backgrounded or killed (FCM `notification` + Android high-priority channel `parent_trip_alerts` + APNs `apns-push-type: alert`).
...
- Parent avatar uploads: primary resilient path is `POST /api/parent/avatar` with Bearer `par.*` (service role, ownership-checked). Object paths stay `{parent.sub}/…` in the public `avatars` bucket. Direct client uploads remain a fallback when a Supabase Auth session is present; storage RLS is owner-scoped INSERT/SELECT/UPDATE/DELETE (`(storage.foldername(name))[1] = auth.uid()`). SELECT is required so Storage `INSERT … RETURNING *` succeeds. Student and primary-parent photos update `students.avatar_url` / `profiles.avatar_url`. Secondary guardian photos update `avatar_url` inside that child's `students.guardians` JSONB (there is no `guardians` table).
...
6. After start, existing **Driver Stop Visit Outcomes** apply at **home** stops. School terminal stops do not open a pick/drop drawer (see **School terminal stops**).
