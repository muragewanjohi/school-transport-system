...
Public apex-only marketing page using `MarketingShell` and `.landing-page` tokens. Four monthly plans: **Starter** (200 students / 3 buses / 1 location, KSh 10,000), **School** (most popular: 600 students / 7 buses / 3 locations, KSh 20,000), **Growth** (1,200 students / 11 buses / 5 locations, KSh 30,000), **Enterprise** (1,200+ students / 11+ buses / unlimited locations, custom). Paid-plan CTAs go to `/request-demo`; Enterprise and help mailto is `info@onthebusapp.com`. A needs calculator recommends the cheapest plan whose caps cover the visitor’s students, buses, and locations. **SMS notifications are excluded** from every plan (push remains in-plan; SMS billed separately). Hero uses `public/stitch/apps_hero_image.png` (admin live tracking, driver app, parent app). This page is marketing copy only and does not change campus-flat-fee billing in the console.

### Request Demo page (`/request-demo`)
Public apex-only marketing page using `.landing-page` tokens. Captures school leads (name, role, school, searchable country combobox with filter-at-top, city/area, WhatsApp/phone with country dial code, required work email, fleet size, preferred time). Validation errors use a human message under the failing field and highlight that input (`--lp-error` / `--lp-error-soft`); raw Zod text such as “Too big” is never shown. On success, the requester immediately gets a Resend confirmation email (“We've received your demo request”), sales is notified, and the visitor is told to wait for an emailed demo school URL and login details after approval. Contact Sales remains a secondary mailto/WhatsApp path.

### Demo request management (`/schools?tab=demos`)
Platform-only tab using the shared admin console shell (light default / dark toggle). Shows pending request count, contact and school details, provisioned demo URL, expiry (default 14 days, editable), requested time/fleet size, submission time, and status actions (Confirm provisions the store; Complete purges it). Status `ready_to_onboard` means the school clicked **Request to go live** — treat as sales follow-up (onboard a paid tenant), not a self-serve paid conversion, and **not** a red inbox badge. **Edit** opens the full-page detail at `/schools/demos/[id]` (lead fields editable while pending) with school-form styling: lead card + access card, theme-aware `form-input` fields, primary/ghost action buttons. While the store is `confirmed` or `ready_to_onboard`, the lead card always shows a **Demo expiry** date (formatted, plus datetime picker, **Save expiry**, and **+14 days** when a live tenant is linked). If the store was purged, the same card says the store was removed and offers **Provision store again**. The Demos table Demo store column always shows the formatted expiry or “Store removed” — never a blank dash for an approved request. The school console banner reads the new `demo_expires_at` immediately. After Confirm & provision, that page shows school URL, admin email/password, and Flutter phone, then emails those details with instructions to request a fresh 15-minute OTP in the app. Confirmed stores expose **Resend access email** (resets the admin password). The platform sidebar and Demo Requests tab badge count **pending (unreviewed) requests only** — confirming a demo clears the notification. `ready_to_onboard` remains in the inbox subtitle (`N ready to onboard`) without a badge.
...
Console form controls (`.form-input`, `.form-select`) must use `--input-bg`, `--text-primary`, `--border-default`, and 12px radius. Do not hardcode navy fills (`rgba(6, 9, 19, …)`), `#FFF` text, or `--bg-glass`. Placeholders use `--text-muted`. This keeps typed text readable in both themes (dark ink on light grey; light ink on dark inset). **Trips** is a top-level school console item with **Today's trip history** (`/trips/history`, same table, no Overrides column) and **Override trip** (`/trips/override`, same table plus **Update Status**). `/routes/today-trips` redirects to Override trip. These pages use the same tokens — never white on `--bg-base`. Each table row is one **trip run**; **Update Status** is hidden for **Completed** rows and the override modal is titled **Override this trip** (Trip first, then Route). Toolbar primary actions (e.g. **Add Route Stop** on `/routes/stops`) use `.btn.btn-primary` with the same 12px radius and matching height as adjacent `.form-input` search and filter controls — never the unpadded browser-default button.
...
- **Admin System Config (`/config`):** Notifications tab toggles include trip start, geofence proximity, boarding/drop-off receipts, **absent at stop**, and **absent at drop-off campus boarding**. Alerts & Geofencing holds proximity radius, min stop dwell, and SMS templates for those events; stop-absent default is `Bus {vehicle_plate} has left stage {stop_name} and {student_name} was marked absent at {time}.` SMS still requires **Enable Global SMS Notifications**. Schools do not enter a Mapbox token — live ETAs use the platform Google Maps key. **BLE Provision PIN** card: shows configured yes/no; Super Admin / Operations Admin can Generate or Rotate (plaintext shown once for copy; hash only stored). Play-review tenant PIN is fixed `654321`.
- **Admin student edit:** Personal Details includes **NFC Card Hash (legacy)** and a read-only **BLE Tag** block (UUID, Major, Minor, MAC, device name, status, provisioned_at) from active `student_beacon_tags`, or empty copy *“No active BLE tag — provision via Driver App.”*
...
- **Mobile Driver View Students:** The Students tab shows the full trip roster (not only the current stop). There is no trip-wide Guardians list and no Pickup/Dropoff Manifest header — tap a student for details and guardians. On student details, each parent/guardian row uses their photo or initials thumbnail on the left (call stays on the trailing phone button). After drop-off campus boarding, non-absent students show **Boarded** (not Dropped off) until marked dropped at their home stop geofence. **Boarded** / **Dropped off** actions still require the child’s stop geofence. When BLE boarding ships ([boarding-technology.md](boarding-technology.md)): show scan-health (Bluetooth / last observation), auto-confirmed boarded/dropped highlights, **uncertain** / signal-unavailable states, and keep manual actions as correction. Prefer “detected / confirmed / needs review” copy — not “tap card” success.
- **Mobile Driver Provision Tag (Android):** Accessible from Home when logged in as driver or conductor. Flow: **required** school Provision PIN → Load template → scan nearby beacons → pick tag → pick student → write tenant iBeacon template + lock device password → success banner *“Tag locked — only OnTheBus can reconfigure this tag.”* Never show or edit student name onto the BLE device name. Trip boarding scan UI stays separate from this screen.
...
- **Admin Trips:** Top-level school console item (not under Routes). Submenu: **Today's trip history** (`/trips/history`) and **Override trip** (`/trips/override`). History is the same daily run table without Overrides. Override is the former Today's Trips board (Update Status + modal). `/routes/today-trips` redirects to Override trip.
...
- **Admin Route planner:** On a selected route, **Trips** is the first (and default) detail tab; **Stops & Geofences** is second. On the Stops tab, **Add existing** copies already-created stages from other routes onto this route (inserted before the end stop). **Add Route Stop** still creates a new stop.
- **Admin Stop editor:** Add Route Stop and Edit Stop open a full page (`/routes/stops/new`, `/routes/stops/[id]/edit`) — not a drawer. Fields: route, stop name, sequence, geofence radius, **Search location** (Google Places), Google Map (click or drag pin for exact lat/lng). Stops are locations on a corridor; pickup vs drop-off is chosen on the student (and by trip direction), not as a stop type. From the route builder, `?route_id=` preselects the current route.
- **Admin Fleet vehicle details:** Bus Maker & Model and Seating Capacity are optional. Last Service Date, Next Service Date, and Insurance Expiry Date are optional. Date pickers default to 2000-01-01 (treated as unset, saved as null). A checkbox **Notify me when service or insurance is due** opts into **fleet-card** alerts at 1 month, 2 weeks, and 1 day before next service or insurance expiry (plus overdue). Those alerts are not emailed to the school admin.
- **Admin student / stop location picker:** Home and stop map search uses Google Places (`GET /api/maps/places`) plus the Maps JavaScript map. It does not call OpenStreetMap Nominatim.
- **Admin student Transit Settings:** Pickup/drop-off radios — **Pickup same as drop-off** (one Stage dropdown, both ids saved) or **Pickup different from drop-off** (pickup + drop-off dropdowns). Same is the default. **Pick up trip** and **Drop off trip** use the same searchable dropdown as pickup/drop-off location, including **None (No Pick up / No Drop off)**.
...
- **Action Sizing:** Inline text indicators use `14px` stroke icons. Buttons, navigation cards, and menu lists use `20px`. Main action indicators (SOS, BLE detected / auto-board confirmed) utilize custom `32px` badges.
