...
Console form controls (`.form-input`, `.form-select`) must use `--input-bg`, `--text-primary`, `--border-default`, and 12px radius. Do not hardcode navy fills (`rgba(6, 9, 19, …)`), `#FFF` text, or `--bg-glass`. Placeholders use `--text-muted`. This keeps typed text readable in both themes (dark ink on light grey; light ink on dark inset). Today's Trips (`/routes/today-trips`) uses the same tokens — never white on `--bg-base`. Toolbar primary actions (e.g. **Add Route Stop** on `/routes/stops`) use `.btn.btn-primary` with the same 12px radius and matching height as adjacent `.form-input` search and filter controls — never the unpadded browser-default button.
