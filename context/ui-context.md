...
- **Mobile Driver Provision Tag (Android):** Accessible from Home overflow / profile when logged in as driver or conductor. Flow: optional school provision PIN → scan nearby beacons (name, MAC, RSSI) → pick tag → pick student → write tenant iBeacon template + lock device password → success banner *“Tag locked — only OnTheBus can reconfigure this tag.”* Never show or edit student name onto the BLE device name. Trip boarding scan UI stays separate from this screen.
