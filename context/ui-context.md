...
- **Parent Mobile App:** Friendly, clean light/dark auto-switching interface that emphasizes maps, child statuses, and clear, non-technical transaction logs. Store listing and home-screen name is **OnTheBus** (iOS + Android). Internal IDs stay `parent_app` / `com.schooltrack.parent_app`. Driver remains a separate app (`OTB Driver`). Parents can open account deletion from the dashboard (`https://onthebusapp.com/delete-account`). The Notifications inbox lists live `notifications` for that parent (campus-exit, stage approach, delay, boarding); the bell badge is unread count, not a placeholder. First launch shows **three onboarding screens** (live map, trip alerts, peace of mind) before login; Skip or Get started is remembered. Profile has a **Log out** button even when no children are linked.
...
- **Mobile Driver Interface (Trip tab):** Full-bleed map plus a persistent bottom control drawer while a trip is active (no scroll column, no trip-in-progress card, no floating Pickup Students FAB):
  ...
  3. **Trip control drawer** — persistent bottom sheet (drag handle, rounded top). Top row: `N of M picked` or `N of M dropped` (pickup = trip-manifest `boarded`; dropoff = `dropped_off`; roster `students.status` Present is not treated as picked), segmented stop-progress bar (completed green / current blue / upcoming gray), geometric ETA + distance to the next stop. Next-stop card: **Approaching {name}** while outside the stage geofence; **You’re at {name}** plus dwell toward min wait once inside. Sequence number, `{n} students waiting` (drop-off: `{n} students to drop`). Primary CTA **Pickup Students** / **DropOff Students** (geofence-gated). Overflow (`…`): **Navigate** and **View Students**. **Skip stop** is locked until `min_stop_dwell_seconds` (default 90 s) after arrival; then marks not visited, remaining Pending → Absent, school admins alerted. **Hold to end trip**. Upcoming-stop peek: the stop after next, with ETA. Arriving inside the next stop geofence still **auto-opens** the ~70–80% boarding drawer (map peek remains) with dwell timer, a per-child **Boarded** (pickup) or **Dropped off** (drop-off) button (no checkbox), **Absent** status with no action button when attendance is absent, tap-name for guardian name/phone + Call (`tel:`), swipe-left to mark absent, **Complete Stop** (remaining Pending → Absent). Leaving after at least one tick → completed + remaining Absent. Leaving with zero ticks before min dwell keeps the stop open; after min dwell → visited + remaining Absent + admin alert. Map legend: next / upcoming / completed / visited / not visited.
- **Mobile Parent Interface:** First-launch onboarding (3 screens) then login. Logged-in home uses a bottom sheet overlay rendering child telemetry status cards that expands to show historical boarding logs. Profile always exposes **Log out**.
- **Mobile Driver View Students:** Overflow **View Students** shows the full trip roster (not only the current stop). A **Guardians** list shows every unique guardian on the trip with `profiles.avatar_url` when present; missing photos use an initials thumbnail. Each student row also lists that child’s guardians. Tap a guardian to call; tap a student for details. **Boarded** / **Dropped off** still requires the child’s stop geofence.
